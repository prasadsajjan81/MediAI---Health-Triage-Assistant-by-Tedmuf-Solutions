...
const AnalysisResult: React.FC<AnalysisResultProps> = ({ markdown, language, patientData, recordId, onBack, onReset }) => {
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex items-center justify-between">
        ...
        {onReset && (
          <button 
            onClick={onReset}
            className="flex items-center space-x-2 bg-teal-50 text-teal-700 px-4 py-2 rounded-xl text-sm font-bold hover:bg-teal-100 transition-all cursor-pointer border border-teal-100"
          >
            <Sparkles size={16} />
      ...
      <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-8 border-t border-slate-100">
        <button
          ...
          className="flex items-center space-x-2 px-6 py-3 bg-slate-100 text-slate-700 rounded-xl font-bold hover:bg-slate-200 transition-all cursor-pointer w-full sm:w-auto justify-center"
        >
          <ChevronUp size={20} />
        ...
        {onReset && (
          <button
            onClick={onReset}
            className="flex items-center space-x-2 px-8 py-3 bg-teal-600 text-white rounded-xl font-bold hover:bg-teal-700 transition-all shadow-lg shadow-teal-600/20 cursor-pointer w-full sm:w-auto justify-center"
          >
            <Plus size={20} />
